...
def outputMetricLAMP(i_train, x_true, observed_pred_global, x_test_global, rmse_global, plt, rmse_local, var_iter_global, RBF_lengthscale, RBF_noise, covar_trace, AIC, BIC):
    n_end = len(i_train)
    ...
    if len(i_95thresh) >= 10:
        for i in range(len(i_95thresh)):
            ...
            vf = np.mean(v_con)
            if np.all(v_con <= vf + dv_2percent) and np.all(v_con >= vf - dv_2percent):
                print("convergence index is " + str(i_95thresh[i]) + " where the total samples is " + str(
                    len(rover_distance)))
                print("convergence rms error is " + str(rmse_global[i_95thresh[i]]))
                print("convergence roving distance is " + str(rover_distance[i_95thresh[i]]))
                print("reduction of error is " + str(max(rmse_global) / rmse_global[i_95thresh[i]]))
                # plotty plot plot converge wrt rms error and distance!
                fig = plt.figure(figsize=(12, 6))
                ax1 = fig.add_subplot(1, 2, 1)
                local_rms = ax1.plot(range(0, len(rmse_local)), rmse_local, color='blue', marker='.', label='local')
                global_rms = ax1.plot(range(0, len(rmse_global)), rmse_global, color='black', marker='*',
                                      label='global')
